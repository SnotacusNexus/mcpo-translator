  ...
  setupRoutes() {
    ...
    const authMiddleware = (req, res, next) => {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        // In a real implementation, you'd validate the token against issued tokens
        // For now, just accept any Bearer token
        console.log(`🔐 Request authenticated with token: ${token.substring(0, 10)}...`);
      } else {
        console.log('⚠️  Request without authentication (auth is optional)');
      }
    ...
    this.app.get('/.well-known/oauth-authorization-server', (req, res) => {
      ...
      res.json({
        ...
        token_endpoint_auth_methods_supported: ['client_secret_post', 'client_secret_basic'],  // Fixed as per guide
    ...
    this.app.post('/oauth/token', (req, res) => {
      const { grant_type, code, refresh_token, code_verifier, client_id, client_secret } = req.body;
      ...
      const expectedClientId = process.env.MCP_CLIENT_ID || 'mcpo-translator-client';
      const expectedClientSecret = process.env.MCP_CLIENT_SECRET || 'mcpo-translator-secret';
